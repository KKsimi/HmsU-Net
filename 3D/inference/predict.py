...
from hmsunet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
from hmsunet.postprocessing.connected_components import load_remove_save, load_postprocessing
from hmsunet.training.model_restore import load_model_and_checkpoint_files
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  overwrite_existing=False,                                                 # model_final_checkpoint
                  all_in_gpu=False, step_size=0.5, checkpoint_name="model_best", # model_best
    ...
    # print(params[0]['state_dict']['deep5.conv.conv.weight'])
    # print(params[0]['state_dict']['deep5.conv.conv.bias'])
...
def predict_cases_fast(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
                       ...
                       all_in_gpu=False, step_size=0.5, checkpoint_name="model_best", #model_final_checkpoint
                       segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False):
    assert len(list_of_lists) == len(output_filenames)
    if segs_from_prev_stage is not None: assert len(segs_from_prev_stage) == len(output_filenames)
